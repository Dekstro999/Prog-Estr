...
void enter(char buffer[], Alumno alumnos[], int *numAlumnos, const char campo[])
{
  ...
  { // Si el usuario ingresó algo que no es solo Enter
    ...
    {
      sscanf(buffer, "%s", alumnos[*numAlumnos].nombre); // Almacenar campo nombre
    }
    else if (strcmp(campo, "apPaterno") == 0)
    {
      sscanf(buffer, "%s", alumnos[*numAlumnos].apPaterno);
    }
    else if (strcmp(campo, "apMaterno") == 0)
    {
      sscanf(buffer, "%s", alumnos[*numAlumnos].apMaterno);
    }
  }
  else
  { // Si el usuario solo presionó Enter
    if (strcmp(campo, "nombre") == 0)
    {
      strcpy(alumnos[*numAlumnos].nombre, "XXXXXX");
    }
    else if (strcmp(campo, "apPaterno") == 0)
    {
      strcpy(alumnos[*numAlumnos].apPaterno, "XXXXXX");
    }
    else if (strcmp(campo, "apMaterno") == 0)
    {
      strcpy(alumnos[*numAlumnos].apMaterno, "XXXXXX");
    }
  }
}
...
void eliminarPrimeraPalabra(char *cadena)
{
  ...
  while (strchr(cadena, ' ') != NULL)
  {
    ...
    if (primerEspacio != NULL) // Si se encuentra al menos un espacio
    {
...
void letrasPaterno(char *inicialP, char *vocalP, char *paterno)
{
  ...
  } while (!validar_letras(paterno)); // Continuar pidiendo el apellido paterno hasta que se ingresen solo letras

  if (strlen(paterno) == 0) // Si la cadena está vacia
  {
    *inicialP = 'X';
    *vocalP = 'X';
    strcpy(paterno, "XXX");
...
void letrasMaterno(char *inicialMatr, char *materno)
{
  ...
  } while (!validar_letras(materno)); // Continuar pidiendo el apellido paterno hasta que se ingresen solo letras

  if (strlen(materno) == 0) // nada
  {
    *inicialMatr = 'X';
    strcpy(materno, "XXX");
  }
...
int obtenerInicialNombre(char *inicial, char *nombre)
{
  ...
  // Verificar si el nombre es "MARIA" o "JOSE"
...
char siguienteConsonante(char *str)
{
  ...
  for (i = 1; i < longitud; i++)
  {
    if ((str[i] >= 'A' && str[i] <= 'Z') || (str[i] >= 'a' && str[i] <= 'z'))
    {
      ...
      {
        return c; // primera consonante encontrada
      }
    }
  }
  return 'X';
